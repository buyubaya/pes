...
function getWithRedirect(sdk, oauthOptions, options) {
  ...
  cookies.setCookie(config.REDIRECT_OAUTH_PARAMS_COOKIE_NAME, JSON.stringify({
    ...
    urls: urls,
    ignoreSignature: oauthParams.ignoreSignature
  }), undefined, oauthParams.domain);

  // Set nonce cookie for servers to validate nonce in id_token
  cookies.setCookie(config.REDIRECT_NONCE_COOKIE_NAME, oauthParams.nonce, undefined, oauthParams.domain);

  // Set state cookie for servers to validate state
  cookies.setCookie(config.REDIRECT_STATE_COOKIE_NAME, oauthParams.state, undefined, oauthParams.domain);
